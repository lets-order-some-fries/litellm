...
def test_init_kwargs_client_metadata_cannot_spoof_authenticated_identity(
    ...
    """Regression (spend redirect): the passthrough cost callback and the batch attribution
    both read user_api_key_user_id/user_api_key_team_id straight from this metadata, so a
    request body that mirrors those internal fields must not be able to bill another user
    or team. They are re-asserted from the authenticated key after the client merge."""
    request = mock_request()
    passthrough_payload = PassthroughStandardLoggingPayload(
        url="https://test.com",
        request_body={},
    ...
    # org and end-user drive their own spend and budget counters, so they are re-asserted too
