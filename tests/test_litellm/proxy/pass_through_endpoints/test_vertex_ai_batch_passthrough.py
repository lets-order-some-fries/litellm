...
class TestVertexAIBatchPassthroughHandler:
    ...
    def test_batch_is_registered_from_the_create_route_only(
        ...
        """Only a POST to the collection route is the create. Every id-scoped route is a
        poll or retrieve, and registering the batch from there would race the create's own
        detached write, so the poll registers nothing at all."""
        response = MagicMock()
