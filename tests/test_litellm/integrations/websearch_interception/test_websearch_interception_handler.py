...
@pytest.mark.asyncio
async def test_execute_search_respects_the_calling_keys_rpm_limit(monkeypatch):
    """The intercepted search consumes the caller's rate limit budget like /v1/search does.

    A key allowed 1 request per minute gets its first search through and is rejected on
    the second, instead of the interception path bypassing the limiter completely.
    """
    import litellm
    ...
    mock_asearch = AsyncMock(return_value=SearchResponse(object="search", results=[]))
    user_api_key_auth = UserAPIKeyAuth(api_key="hashed-sk-1234", rpm_limit=1)
    rate_limiter = _PROXY_MaxParallelRequestsHandler_v3(internal_usage_cache=InternalUsageCache(dual_cache=DualCache()))
    ...
    assert mock_asearch.await_count == 1
