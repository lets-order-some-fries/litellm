...
def refresh_model_cost_map(url: str) -> int:
    ...
    The refresh replaces the whole catalog, which on its own discards everything
    registered at runtime: the deployment ``model_info`` the Router registers
    from ``model_list``, and pricing overrides passed to ``register_model``. Both
    are re-applied on top of the freshly fetched catalog so a price data reload
    only updates pricing rather than erasing operator-supplied model metadata.
    ...
    anything is re-applied so it describes the price data alone.
    """
