...
_live_routers: "weakref.WeakSet[Router]" = weakref.WeakSet()  # mutable-ok: identity set of live routers
...
class Router:
    ...
    def _deployment_model_cost_payload(deployment: Deployment) -> dict:  # mutable-ok: cost-map entry
        ...
        """
        model_info: dict = deployment.model_info.model_dump(exclude_none=True)  # mutable-ok: built in place
    ...
    def _register_deployment_in_model_cost(
        *,
        ...
        backend_keys = Router._backend_cost_map_keys(model=model, custom_llm_provider=custom_llm_provider)
        ...
        shared_model_info = shared_backend_model_info(model_info)
        existing_shared_mode = (cast(dict | None, litellm.model_cost.get(backend_key, {})) or {}).get("mode")
        deployment_mode = shared_model_info.get("mode")
        ...
        is_responses_to_chat_downgrade = existing_shared_mode == "responses" and deployment_mode == "chat"
        would_clear_existing_mode = existing_shared_mode is not None and deployment_mode is None
