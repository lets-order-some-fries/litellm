...
ON CONFLICT (api_key, session_id, router_name) DO UPDATE SET
    ...
    models = t.models || jsonb_build_object($5, jsonb_build_object(
        'at', (CASE WHEN {_CACHE_TOUCHED}
               THEN GREATEST(COALESCE((t.models -> $5 ->> 'at')::float8, 0), EXTRACT(EPOCH FROM $6::timestamp))
               ELSE COALESCE((t.models -> $5 ->> 'at')::float8, EXTRACT(EPOCH FROM $6::timestamp)) END),
